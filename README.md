...
https://hub.docker.com/r/dockercoolexp/movie_recommender

Pull the docker image and create a container. Copy the .prs file mentioned below into the container. The container should automatically reload. Otherwise stop and rerun.

docker run --name test -p 8010:8010 dockercoolexp/movie_recommender:v1

## Note: The Content based recommender doesn't return any results because of two reasons:
1. Lower amount of descriptions. The movname.pkl file is generated by the movname notebook. The number 3997 in the notebook can be replaced by a bigger number to generate a bigger movname.pkl and hence greater number of movies.
2. The similarity threshold is higher. Lowering the threshold may help.
https://drive.google.com/file/d/1SE8yTpDQnjv_peSetVTN3wbtJOdyXINp/view?usp=drive_link
https://drive.google.com/file/d/1SvWw670YpjrRBbmm0iZRMkYr54SaMfIM/view?usp=drive_link
